...
//17、输入N个人的语(chinese)数(math)外(english)考试成绩， 请输出总分最低的学生和单科最低的学生，相同的成绩也要输出(按照原来的顺序）
// 输入：一个整数n表示n个人的考试成绩（1<n<=1000),随后有n行，每行依次是这个学生的姓名，语文成绩，数学成绩，外语成绩（都是整数）
// 输出：首先输出语文最低的人（可以并列），然后是数学最低的,外语最低的，最后输出总分最低的人（可以并列）
// 具体格式参考输出样例输入样例5tom1009998alice1
